...
describe("api service", () => {
  ...
  test("fetchReportsData keeps the requested month when fallback is disabled", async () => {
    const { apiClient, fetchReportsData } = loadApiModule();

    apiClient.get
      .mockResolvedValueOnce({
        data: { summary: { totalSpending: 0, transactionCount: 0, highest: 0 } },
      })
      .mockResolvedValueOnce({ data: { categoryData: [] } })
      .mockResolvedValueOnce({ data: { trendData: [{ month: "Apr", total: 450 }] } });

    const response = await fetchReportsData(4, "2026-05");

    expect(apiClient.get).not.toHaveBeenCalledWith("/expenses/user/4");
    expect(response).toEqual({
      summary: { totalSpending: 0, transactionCount: 0, highest: 0 },
      categoryData: [],
      trendData: [{ month: "Apr", total: 450 }],
  ...
  test("fetchReportsData falls back to the latest expense month when enabled and the selected month is empty", async () => {
    const { apiClient, fetchReportsData } = loadApiModule();
    ...
    apiClient.get
      ...
      .mockResolvedValueOnce({
        data: {
          expenses: [
            { id: 1, date: "2026-02-10", amount: 100 },
            { id: 2, date: "2026-04-18", amount: 450 },
          ],
        },
      })
      .mockResolvedValueOnce({
        data: { summary: { totalSpending: 450, transactionCount: 1, highest: 450 } },
      })
      .mockResolvedValueOnce({
        data: { categoryData: [{ category: "Food", total: 450, percentage: 100 }] },
      });

    const response = await fetchReportsData(4, "2026-05", {
      allowMonthFallback: true,
    });

    expect(apiClient.get).toHaveBeenCalledWith("/expenses/user/4");
    expect(apiClient.get).toHaveBeenCalledWith("/reports/summary/4?month=2026-04");
    expect(apiClient.get).toHaveBeenCalledWith("/reports/category/4?month=2026-04");
    expect(response).toEqual({
      summary: { totalSpending: 450, transactionCount: 1, highest: 450 },
      categoryData: [{ category: "Food", total: 450, percentage: 100 }],
      trendData: [{ month: "Apr", total: 450 }],
      selectedMonth: "2026-04",
    });
  });
