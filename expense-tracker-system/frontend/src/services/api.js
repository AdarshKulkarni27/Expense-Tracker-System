...
function hasReportActivity(summary, categoryData) {
  ...
  return (
    ...
    Number(summary?.totalSpending || 0) > 0 ||
    Number(summary?.highest || 0) > 0
  );
}
...
function getLatestExpenseMonth(expenses) {
  return expenses.reduce((latestMonth, expense) => {
    const expenseMonth = normalizeMonthValue(expense?.date);

    if (!expenseMonth) {
      return latestMonth;
    }

    return !latestMonth || expenseMonth > latestMonth
      ? expenseMonth
      : latestMonth;
  }, "");
}
...
export async function fetchReportsData(userId, month, options = {}) {
  const { allowMonthFallback = false } = options;
  ...
  let summary = unwrapObject(summaryResponse.data, ["data", "summary"]);
  let categoryData = unwrapCollection(categoryResponse.data, [
    "data",
    "categoryData",
    "categories",
    "content",
  ]);
  const trendData = unwrapCollection(trendResponse.data, [
    "data",
    "trendData",
    "trend",
    "content",
  ...
  if (allowMonthFallback && !hasReportActivity(summary, categoryData)) {
    const expensesResponse = await api.get(`/expenses/user/${userId}`);
    const latestExpenseMonth = getLatestExpenseMonth(
      normalizeExpenses(expensesResponse.data)
    );

    if (latestExpenseMonth && latestExpenseMonth !== month) {
      const [fallbackSummaryResponse, fallbackCategoryResponse] =
        await Promise.all([
          api.get(`/reports/summary/${userId}?month=${latestExpenseMonth}`),
          api.get(`/reports/category/${userId}?month=${latestExpenseMonth}`),
        ]);

      summary = unwrapObject(fallbackSummaryResponse.data, ["data", "summary"]);
      categoryData = unwrapCollection(fallbackCategoryResponse.data, [
        "data",
        "categoryData",
        "categories",
        "content",
      ]);
      selectedMonth = latestExpenseMonth;
    }
  }
