...
function Reports({ currentPath, onNavigate, authSession, onLogout }) {
  ...
  useEffect(() => {
    ...
    async function loadReports() {
      try {
        setIsLoading(true);
        const shouldAllowMonthFallback =
          !hasResolvedInitialMonth && selectedMonth === defaultMonth;
        const data = await fetchReportsData(userId, selectedMonth, {
          allowMonthFallback: shouldAllowMonthFallback,
        });
        ...
        if (shouldAllowMonthFallback) {
          setHasResolvedInitialMonth(true);
        }
        if (data.selectedMonth && data.selectedMonth !== selectedMonth) {
  ...
  }, [defaultMonth, hasResolvedInitialMonth, selectedMonth, userId]);
